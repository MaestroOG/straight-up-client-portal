...
export async function getUnreadCountsForTasks(taskIds, userId) {
    ...
    const counts = await TaskComment.aggregate([
        { $match: { taskId: { $in: taskIds }, readBy: { $nin: [userId] } } },
...
export async function getLastThreeTaskCommentByTask(taskId) {
    try {
        await connectDB();
        const comment = await TaskComment.find({ taskId: taskId })
        ...
        console.error("Error fetching last three comment:", error);
        return [];
    }
}